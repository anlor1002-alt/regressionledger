// `rl demo` / `npx regressionledger demo`: a self-contained, no-Claude-needed
// simulation of the doom loop RegressionLedger breaks. Everything runs inside a
// throwaway temp directory through the real hook code paths — your settings,
// ledger, and repo are never touched.
//
...
/** Run the narrated doom-loop demo. Returns a CLI exit code. */
export function runDemo() {
  ...
  try {
    ...
    console.log(color.dim(`  Sandbox: everything below happens in ${cwd} — your setup is untouched.`));
    line();
    ...
    line();
    const denied = guarded.json?.hookSpecificOutput?.permissionDecision === 'deny';
    if (denied) {
    ...
    console.log(
      color.dim('  Everything above ran through the real hook entry points') +
        color.dim(' (PreToolUse/PostToolUse) — no mocks.')
    );
    console.log(
      `  ${color.bold('Install for real:')} npx regressionledger init` +
        color.dim('   (and `rl uninstall` removes every trace)') +
        '\n'
    );
    return denied ? 0 : 1;
  } finally {
